...
import { getAutomationEntityFields, createAutomation, listAutomationsByUserId } from "../db/automationRepository.js";
...
const validateConditions = (conditions, fieldsMap) => {
  ...
  for (const condition of conditions) {
    ...
    if (!entity || !field || !operator) {
      return "Each condition requires entity, field and operator";
    }
    ...
    if (!fieldDef) {
      return `Field ${field} does not exist in ${entity}`;
    }

    const allowed = allowedOpsByType[fieldDef.dataType] || STRING_OPERATORS;
    if (!allowed.includes(operator)) {
      return `Operator ${operator} is not valid for ${fieldDef.dataType} field ${field}`;
    }
...
export const getAutomationBuilderMetadataForUser = async (authHeader) => {
  ...
  const [fields, templates] = await Promise.all([
    ...
    listMailTemplatesByUserId(user.id),
  ]);
...
export const listAutomationsForUser = async (authHeader) => {
  const user = await getAuthorizedUser(authHeader);
  if (!user) return { status: 401, body: { message: "unauthorized" } };

  const automations = await listAutomationsByUserId(user.id);
...
export const createAutomationForUser = async (authHeader, payload) => {
  ...
  if (conditionValidationError) {
    return { status: 400, body: { message: conditionValidationError } };
  }

  if (!["AND", "OR"].includes(conditionLogic)) {
    return { status: 400, body: { message: "Condition logic must be AND or OR" } };
  }

  if (["Send Mail", "AI Generate (Auto Reply)", "AI Generate (Draft)"].includes(actionType)) {
    if (!mailTemplateId) {
      return { status: 400, body: { message: "Mail template is required for selected action" } };
    }

    const templates = await listMailTemplatesByUserId(user.id);
    const exists = templates.some((template) => template.id === mailTemplateId);
    if (!exists) {
      return { status: 400, body: { message: "Selected mail template does not exist" } };
    }
  }
